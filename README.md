...
本项目在桌面端侧边栏中集成了 Google AdSense 广告（仅桌面端显示，移动端不展示）。

**为什么需要广告？**

DuckMail 是一个完全免费、开源的项目，但后端服务器的运营成本是真实存在的：
- 邮件服务器的持续运行和维护
- 域名续费和 SSL 证书
- 服务器带宽和存储费用

广告收入将 **用于覆盖服务器运营成本**，帮助项目长期稳定运行。如果你 fork 了本项目用于自部署，可以自行移除或替换为自己的广告代码。
